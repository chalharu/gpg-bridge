...
- Settings では保存済みサーバー URL を読み取り専用で確認できる想定とし、切り替えは再登録フローへ委ねる。
...
- Purpose: アプリテーマの切り替えと、現在の接続先サーバーの確認を行う。
- Key UI elements: AppBar「設定」、接続サーバー表示カード、テーマ見出し、3 つの RadioListTile。
- Primary actions: システム設定 / ライト / ダークの選択。接続サーバーは読み取り専用で表示する。
- State / notes: テーマは `themeModeStateProvider` 経由で永続化する。提案 UI では登録時に保存したサーバー URL を Settings に表示し、切り替えが必要な場合は再登録フローへ誘導する。現在の実装ではテーマ切替のみが存在する。
...
- State / notes: FCM 受信時に push 表示される一時画面。読み込み中、エラー、要求未発見に対応。詳細カードは `ハッシュアルゴリズム -> 鍵ID -> ハッシュ値` の順で表示し、残り 60 秒未満でバナーが警告色になり、期限切れ時は SnackBar を出して自動で閉じる。

## UI Improvement Proposals

- Register はサーバー URL のリアルタイム検証状態を入力欄直下に集約し、無効値では登録ボタンを無効化する。
- Register の処理中表示には選択済みサーバー URL を併記し、誤接続の不安を減らす。
- Settings には読み取り専用の「接続サーバー」カードを置き、登録後でも現在の接続先を確認できるようにする。
- Sign Request は長いハッシュ値を最後に寄せ、先にアルゴリズムと鍵 ID を見せることで承認判断を速くする。
- Keys の 2 タブはどちらも pull-to-refresh 対応であることをモックと文書上で明示し、一覧系 UI の一貫性を上げる。
