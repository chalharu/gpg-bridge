...
未登録端末の初期到達先です。中央の `Complete registration` ボタンと、成功後に token refresh listener を開始する流れを示しています。現在の実装では接続先の登録先サーバーはこの画面で入力させず、`API_BASE_URL` の dart-define によるビルド時設定を `ApiConfig.baseUrl` として読み込んだ値を使います。
...
- 現在の実装では Register が呼ぶ `/device` への初期登録先は、Register 画面のユーザー入力ではなく `ApiConfig -> httpClientProvider(Dio) -> deviceApiProvider -> DeviceRegistrationService` の経路で渡される `API_BASE_URL` に固定される。
- 提案設計では Register 表示時に `API_BASE_URL` を初期値としてサーバー入力欄へ表示し、ユーザーが必要に応じて別サーバーへ上書きできるようにする。
- 提案設計では登録前に選択済みサーバーを検証し、登録成功後はその値を永続化して以後の API 呼び出しに利用する。
...
- Key UI elements: AppBar「Register」、中央の登録ボタン、処理中インジケータ。サーバー URL 入力欄は持たない。
- Primary actions: Complete registration 実行。
- State / notes: 成功後に token refresh listener を開始。失敗時は SnackBar 表示。未登録時は他ルートへ進めない。登録先サーバーは `API_BASE_URL` の dart-define を `ApiConfig.baseUrl` として読み込んだ build-time config により決まり、Register 画面上での切り替えや直接入力は行わない。実際の登録リクエストは `httpClientProvider` がその base URL を持つ `Dio` を組み立て、`deviceApiProvider` 経由で `DeviceRegistrationService.register()` から `POST /device` を送る。

#### Register 改善案 [提案]

- 目的: 初回登録時にユーザーが登録先サーバーを確認または上書きできるようにし、ビルド時設定だけに依存しない運用を可能にする。
- 追加 UI: `API_BASE_URL` を初期値として表示するサーバー URL 入力欄、入力値の検証結果、`Complete registration` ボタン。
- 検証方針: 少なくとも `https://` を前提に URL 形式を確認し、登録前に選択済みサーバーへ疎通確認を行う。検証用エンドポイントやタイムアウト値の詳細は後続の実装タスクで確定する。
- 想定フロー: ユーザーがサーバー URL を確定し、検証成功後にその URL 向けに `POST /device` を実行する。登録成功時は選択済みサーバーを永続化し、以後の API 呼び出しは保存値を優先して利用する。
- スコープ外: 登録済み端末が後から接続先サーバーを切り替える機能は本提案には含めない。切り替えが必要な場合は既存登録を解除して再登録する別フローとして扱う。
...
- Register のランタイムサーバー選択は現時点では未実装であり、この文書では現行実装と提案設計を分けて記載している。
