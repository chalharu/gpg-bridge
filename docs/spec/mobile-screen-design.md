...
未登録端末の初期到達先です。中央の `Complete registration` ボタンと、成功後に token refresh listener を開始する流れを示しています。接続先のペアリングサーバーはこの画面で入力させず、`API_BASE_URL` の dart-define によるビルド時設定を `ApiConfig.baseUrl` として読み込んだ値を使います。
...
- Register が呼ぶ `/device` への初期登録先は、Register 画面のユーザー入力ではなく `ApiConfig -> httpClientProvider(Dio) -> deviceApiProvider -> DeviceRegistrationService` の経路で渡される `API_BASE_URL` に固定される。
