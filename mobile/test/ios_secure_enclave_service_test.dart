...
import 'package:gpg_bridge_mobile/security/ios_secure_enclave_service.dart';
...
void main() {
  ...
  test(
    ...
    () async {
      final service = IosSecureEnclaveService(channel);

      await service.generateKeyPair(alias: IosSecureEnclaveAliases.deviceKey);

      final signature = await service.sign(
        alias: IosSecureEnclaveAliases.deviceKey,
  ...
  test('service wraps PlatformException from each method', () async {
    ...
    await expectLater(
      () => service.generateKeyPair(alias: IosSecureEnclaveAliases.deviceKey),
      throwsA(isA<IosSecureEnclaveException>()),
    );
    ...
    await expectLater(
      () => service.getPublicKeyJwk(alias: IosSecureEnclaveAliases.deviceKey),
      throwsA(isA<IosSecureEnclaveException>()),
    );
  });
  ...
  test('exception toString includes cause when present', () {
    final error = IosSecureEnclaveException('failed', cause: Exception('boom'));
  ...
  test('exception toString omits cause when absent', () {
    final error = IosSecureEnclaveException('failed');

    expect(error.toString(), 'IosSecureEnclaveException: failed');
  });
  ...
  test('service rejects e2e alias for sign and verify', () async {
    ...
    await expectLater(
      () => service.sign(
        alias: IosSecureEnclaveAliases.e2eKey,
        data: utf8.encode('hello'),
      ),
      throwsA(isA<IosSecureEnclaveException>()),
    );
