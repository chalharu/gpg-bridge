...
mod tests {
    ...
    /// When another test already initialized a subscriber, `try_init` returns
    /// a "subscriber already set" error, NOT the "must be either" validation
    /// error.  This kills the "delete match arm json" mutant.
    #[test]
    fn init_tracing_json_does_not_reject_format() {
        let config = AppConfig {
            server_host: "127.0.0.1".to_owned(),
            server_port: 3000,
            database_url: "postgres://localhost:5432/gpg_bridge".to_owned(),
        ...
        let result = init_tracing(&config);
        // Either succeeds (first test to run) or fails because subscriber
        // was already set — both are acceptable. The critical assertion is
        // that it does NOT fail with the validation error that would appear
        // if the "json" arm were deleted.
        if let Err(e) = &result {
            let msg = e.to_string();
            assert!(
                !msg.contains("must be either"),
                "'json' must be accepted as a valid format, got: {msg}"
            );
        }
    }
}
