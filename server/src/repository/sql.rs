...
macro_rules! impl_common_sql_repositories {
    ($repo:ty, $count_ty:ty) => {
        #[async_trait]
        impl JtiRepository for $repo {
        ...
        #[async_trait]
        impl AuditLogRepository for $repo {
        ...
        #[async_trait]
        impl CleanupRepository for $repo {
        ...
        #[async_trait]
        impl SigningKeyRepository for $repo {
        ...
        #[async_trait]
        impl ClientRepository for $repo {
            ...
            async fn client_exists(&self, client_id: &str) -> anyhow::Result<bool> {
                let count = sqlx::query_scalar::<_, $count_ty>(
        ...
        #[async_trait]
        impl PairingRepository for $repo {
        ...
        #[async_trait]
        impl ClientPairingRepository for $repo {
            ...
            async fn delete_client_pairing_and_cleanup(
                ...
                if pairing_deleted {
                    let remaining = sqlx::query_scalar::<_, $count_ty>(
...
impl_common_sql_repositories!(crate::repository::PostgresRepository, i64);
impl_common_sql_repositories!(crate::repository::SqliteRepository, i32);
