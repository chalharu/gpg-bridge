...
impl_signature_repository!(
    ...
    backend = "sqlite",
    count_scalar = i32,
    count_pending_for_pairing_sql = "SELECT COUNT(*) FROM requests WHERE status IN ('created', 'pending') AND EXISTS (SELECT 1 FROM json_each(requests.client_ids) WHERE json_each.value = $1) AND json_extract(requests.pairing_ids, '$.\"' || $1 || '\"') = $2",
    get_pending_for_client_sql = "SELECT request_id, status, expired, signature, client_ids, daemon_public_key, daemon_enc_public_key, pairing_ids, e2e_kids, encrypted_payloads, unavailable_client_ids FROM requests WHERE status = 'pending' AND expired > datetime('now') AND EXISTS (SELECT 1 FROM json_each(requests.client_ids) WHERE json_each.value = $1) AND NOT EXISTS (SELECT 1 FROM json_each(requests.unavailable_client_ids) WHERE json_each.value = $1)",
    add_unavail_select_sql =
        "SELECT unavailable_client_ids, client_ids, status FROM requests WHERE request_id = $1",
    is_kid_in_flight_sql = "SELECT EXISTS(SELECT 1 FROM requests, json_each(requests.e2e_kids) AS je WHERE requests.status IN ('created', 'pending') AND je.value = $1)",
    is_kid_in_flight_scalar = i32,
);
